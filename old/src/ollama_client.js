...
// Parse chunks returned by the streaming reader
// Apparently, multiple json packets can be there in one chunk!
function split_parse_chunk(chunk) {
    let decoder = new TextDecoder("utf-8")
    let text = decoder.decode(chunk)
    // Here text may contain multiple json packages
    // Following is dirty. Look away!
    text = text.replace(new RegExp('}\n{\s*"', 'g'), '}<==break==>{"')
    let parts = text.split("<==break==>")
...
export async function llm_generate(text, push_text=null) {
    ...
    while (true) {
        ...
        let response = split_parse_chunk(chunk.value)
        if (response != "") {
